...
int main() {
	...
	if (cap.isOpened()) // check if we succeeded 
	{
		...
		for (int i = 0; i < objects.size(); i++) {
			match.push_back(matchImages(ratio, false, NORM_L2, obj_desc[i], frame_desc, obj_key[i], frame_key));
		...
		for(;;) { 
			...
			for (auto& obj : objects) {
				...
				vertex[k] = findProjection(obj, frame, obj_track_points[k], std::vector<Point2f>(shift_points.begin() + start, shift_points.begin() + end));

				frame = drawBox(frame, objects[k], vertex[k], color[k]);
